...
{
    public sealed record CommandPatchDepartment : IRequest<VMBASE_RES<ORG_DEPARTMENT>>
    {
        public Guid Key { get; set; }
        public Delta<ORG_DEPARTMENT> Args { get; set; }
        public string UserName { get;}
        public CommandPatchDepartment(Guid key, Delta<ORG_DEPARTMENT> args, string username )
        {
            Key = key;
            Args = args;    
            UserName = username;
        }
        internal sealed class CommandPatchDepartmentHandler : IRequestHandler<CommandPatchDepartment, VMBASE_RES<ORG_DEPARTMENT>>
        {
            private readonly IUtilityCommon _util;
            private readonly INLogCommon _nLog;
            private readonly IUnitOfWork _uow;
            private readonly IMapper _mapper;
            public CommandPatchDepartmentHandler(IUtilityCommon util, INLogCommon nLog, IUnitOfWork uow, IMapper mapper)
            {
                this._util = util;
                this._nLog = nLog;
                this._uow = uow;
            ...
            public async Task<VMBASE_RES<ORG_DEPARTMENT>> Handle(CommandPatchDepartment request , CancellationToken cancellationToken = default)
            {
